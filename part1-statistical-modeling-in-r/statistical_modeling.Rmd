...
```{r, fig.height=3.75, fig.width= 7, dev='svg', message=FALSE}
...
- $\bf{x_{\mbox{SATQ}}}$ = vector of SATQ scores of length $n$ (*independent variable* or *covariate*)

$$y_i = \beta_0x_{0,i}+\beta_{\mbox{SATQ}}x_{\mbox{SATQ},i}+\epsilon_i, \ \ i = 1, ..., n, \\
...
- $\beta_0$ and  $\beta_{\mbox{SATQ}}$ are scalars (i.e., of length 1) and called *regression coefficients* or *parameters* ( $\sigma^2_{\epsilon}$ is also a parameter). $\beta_0$ is also known as the *intercept*.
...
```{r, fig.height=5.5, fig.width=5.5, dev='svg', message=FALSE}
...
--
- Why is estimate of `SATQ_c` reduced by 35% (was `0.02453`)?
--
...
- Open `exercises/exercise_1_MR.Rmd` (or the exercise handout or  `exercises/exercise_1_MR.nb.html` for a nicer format of the instruction).
...
- Exercise 1: Create one simple linear regression model with murder rate as dependent variable and each of the other variables as independent variable.
    - Which of the three variables shows a significant effect on murder rates?
- Exercise 2: Create one overall model with murder rate as dependent variable and the three other variables as independent variables (without interactions).
...
.pull-right[
```{r, fig.width=5, fig.height=5, dev='svg'}
...
- Alternatively, we can inspect interaction by transforming one variable into numerical variable!
...
.pull-left[
```{r, fig.width=4.5, fig.height=4.5, dev='svg', echo=FALSE}
...
- Linear transformation of variables affects interpretation of estimates, but not overall adequacy of model
    ...
    - Multicollinearity might affect estimates and precision
...
.pull-right[
```{r, fig.width=4.5, fig.height=4.5, dev='svg'}
