---
title: "Exercise 1: Pooling (Solution)"
...
# Exercise 2: Analysing more conditions using complete pooling and no-pooling approach
...
- Your task is to calculate the regression parameter (and potentially also the intercept) for within-subject condition (i.e., relationship of `dv` and `if_A_then_B` and `B_given_A` for each level of `rel_cond`).
- There are different ways how to interpret complete pooling. Either one ignores individual differences or one aggregates across them. Can you find the different ways for implementing no pooling here?
...
- Your task is to calculate the regression parameter (and potentially also the intercept) for each participant and within-subject condition (i.e., relationship of `dv` and `if_A_then_B` and `B_given_A` for each `p_id` and level of `rel_cond`).
